        ...
        for j, mode in enumerate(part_mode):
            ...
            if isinstance(mode, int):
                image = Image.fromarray(
                    process_part(origin_image, mode, playback=playback, resize=True)
                )
                ...
                for name, img in enumerate(cards):
                    Image.fromarray(img).save(
                        str(path_save_file.with_stem(path_save_file.stem + f"_{name}"))
                    )
...
if __name__ == "__main__":
    ...
    paths = path_manager.search(
        "videos",
        video_name="/home/yy/Coding/datasets/Clash-Royale-Dataset/videos/segment_test/WTY_20240506/1.mp4",
        regex="^\d+.mp4$",
    )
    for path in paths:
