...
class Generator:
    ...
    def __init__(
        ...
        if self.recognize_names is None:
          ...
          self.recognize_names = yaml.class_id_to_name.values()
        ...
        if queen:
            for i in range(2):  # is enermy?
                for j in range(2):  # left or right
                    p = random.random()
                    if p > sum(
                        list(tower_generation_ratio.values())
                    ):  # generating tower ruin in background_itmes/ruin{i}.png
                        path = self._sample_elem(
                            self.path_manager.search(
                                subset="images",
                                part="segment",
        ...
        for c in cs:
            ...
            if c == "bar":  # update xy, xy is junction of bar-level and bar
                ...
                bar_level = self._build_unit_from_path(
                    self._sample_elem(paths_bar_level), xy_, level, None, "right_center"
                )
