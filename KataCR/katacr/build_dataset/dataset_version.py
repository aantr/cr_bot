...
class DatasetManager:
    ...
    def save(self, part=["annotation", "segment"]):
        ...
        for pt in part:
            ...
            paths = sorted(
                list(root_path.glob(pt + "*.csv")),
                key=lambda p: int(p.stem.split("_")[1].split(".")[1]),
            )
            if len(paths) == 0:
                new_version = "0.1"
                ...
                for i in data:
                    ...
                    if not (old == new).all():
                        if same_flag:
                            ...
                            note_file = (
                                root_path / f"{pt}_v{new_version}_update.txt"
                            ).open("w")
                            s = (
                                " " * 30
                                + f"{'Old (v'+old_version+')':>16} -> {'New (v'+new_version+')':>16} {'Count':>8}"
                            )
                            note_file.write(s + "\n")
                            print(s)
                        diff_count += 1
