...
def get_features(path_features: Path) -> Sequence[np.ndarray]:
    ...
    for path in path_features.iterdir():
        ...
        if path.is_file() and path.name[-3:] == "jpg":
            feature = {"feature": None, "x_loc_rate": None, "y_loc_rate": None}
...
def check_feature_exists(image: np.ndarray, features: Sequence[dict]) -> bool:
...
class Cutter:
    ...
    def split_episodes(self, path_video: Path, show=True, interval=10):
        ...
        print(
            colorstr("[Video info]")
            + f" fps={fps:.2f}, duration={second2str(duration)}"
        )
        ...
        for frame in bar:
            ...
            if start_frame != -1 and (
                ...
                or (
                    (frame - start_frame) * interval / fps > 15 and start_flag
                )  # WE think each episode length will longer than 30s
                ...
                ffmpeg_extract_subclip(
                    str(path_video),
                    start_frame * interval / fps,
                    frame * interval / fps + 1 + dt_end,
                    str(path),
                )
                print(
                    f"Split episode{episode_num} in {second2str(start_frame*interval/fps)}~{second2str(frame*interval/fps)}"
                )
                start_frame = -1
            if start_frame == -1 and start_flag:
                ...
                start_frame = frame
            bar.set_description(
                f"Process {episode_num} episode"
                + f", start at {second2str(start_frame*interval/fps)}"
                if start_frame != -1
                else ""
            )
            if show:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--path-video",
        type=cvt2Path,
        default="/home/yy/Coding/datasets/Clash-Royale-Dataset/videos/fast_pig_2.6/WTY_20240512_134920_golem_ai.mp4",
    )
