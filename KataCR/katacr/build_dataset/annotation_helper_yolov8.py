...
@Desc    : Use combo yolov8 to help annotate labels.
"""
import sys
...
class AnnotationHelper:
    ...
    def _check_img_path(self, p: Path):
        ...
        if path_json.exists():
            print(
                f"Found path {p.parent} image {p.name} has json file {path_json.name}, skip it!"
            )
            return False
        return True
    ...
    def process(self):
        paths = self.path_manager.search(
            "images", 2, self.args.video_name, self.args.episode, regex=r"^\d+.jpg"
        )
        ...
        for p in bar:
            ...
            for i in range(pbox.shape[0]):
                ...
                if (x[2] < 280 and x[3] < 80) or (x[0] > 390 and x[3] < 120):
                    continue
                shapes.append(
                    {
                        ...
                        "group_id": None,
                        "description": (
                            f"track id: {int(x[-4])}, " if len(x) == 8 else ""
                        )
                        + f"confidence: {float(x[-3]):.4f}",
