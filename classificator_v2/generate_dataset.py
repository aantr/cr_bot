...
def process_single_image_as_class(image_path, class_name, output_train_path, output_val_path, 
                                augment_count=8, val_ratio=0.2):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Генерация датасета: каждый файл как отдельный класс с аугментациями')
    ...
    parser.add_argument('--max-search-images', type=int, default=None, 
    ...
    for img_path in all_images:
        ...
        class_name = "".join(c for c in filename_without_ext if c.isalnum() or c in (' ', '-', '_')).strip()
    ...
    train_classes = len([d for d in os.listdir(train_output_path) 
                        if os.path.isdir(os.path.join(train_output_path, d))]) if os.path.exists(train_output_path) else 0
    val_classes = len([d for d in os.listdir(val_output_path) 
                      if os.path.isdir(os.path.join(val_output_path, d))]) if os.path.exists(val_output_path) else 0
    ...
    print(f"  └── search_dataset/  # Датасет для поиска ({num_search_images} изображений)")
