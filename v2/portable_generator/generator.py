...
if __package__:
    from .support import PathManager, build_label2colors, plot_box_PIL, write_yolo_annotations
    from .labels import configure_class_names, idx2unit, unit2idx
    from .states import idx2state
    from .generation_config import (
        ...
        unit_scale,
        unit_stretch,
        tower_intersect_ratio_thre,
        bar_intersect_ratio_thre,
        tower_generation_ratio,
        king_tower_generation_ratio,
    )
else:  # Allows: python generator.py
    from support import PathManager, build_label2colors, plot_box_PIL, write_yolo_annotations
    from labels import configure_class_names, idx2unit, unit2idx
    from states import idx2state
    from generation_config import (
    ...
    aug2unit,
    alpha_transparency,
    background_augment,  # augmentation
    component_prob,
    component2unit,
    component_cfg,
    important_components,
    option_components,
    bar_xy_range,  # component configs
    item_cfg,
    drop_box,
    background_item_list,  # background item
    unit_scale,
    unit_stretch,  # affine transformation
    tower_intersect_ratio_thre,
    bar_intersect_ratio_thre,
    tower_generation_ratio,
    king_tower_generation_ratio,
...
def add_filter(
    ...
    if xyxy is None:
        xyxy = (0, 0, img.shape[1], img.shape[0])
    proc_img = img
...
class Unit:
    def __init__(
        ...
        if name is not None or (cls is not None and states is not None):
            ...
            if isinstance(cls, str):
                if cls in drop_box or drop or cls not in unit2idx:
                    self.cls = -1  # background items
                else:
                    self.cls = unit2idx[cls]
            else:
                self.cls = cls
        ...
        if augment:  # scale and stretch
            if self.cls_name in unit_scale:
                ...
                if random.random() < prob:
                    ...
                    size = (np.array((img.shape[1], img.shape[0])) * scale).astype(
                        np.int32
                    )
            ...
            if self.cls_name in unit_stretch:
                ...
                if random.random() < prob:
                    ...
                    size = (np.array((img.shape[1] * stretch, img.shape[0]))).astype(
                        np.int32
                    )
                    img = np.array(Image.fromarray(img).resize(size))
        ...
        self.xyxy = np.array(
            (xy[0] - w // 2, xy[1] - h, xy[0] + (w + 1) // 2, xy[1]), np.float32
        )  # xyxy relative to background
        if (
            self.cls_name in ["text", "bar", "circle"] + spell_unit_list
        ):  # if text or spell units, clip the out range
            self.xyxy = np.array(
                (
                    max(self.xyxy[0], 0),
                    max(self.xyxy[1], 0),
            ...
            # clip the image
            xyxy_relative = (
                self.xyxy - np.array([xy[0] - w // 2, xy[1] - h] * 2)
            ).astype(np.int32)
            self.xyxy_relative = xyxy_relative
            img = img[
                xyxy_relative[1] : xyxy_relative[3],
                xyxy_relative[0] : xyxy_relative[2],
                :,
            ]
            ...
            for i in range(2):
                if self.xyxy[i] < 0:
                    dxy[i] = -self.xyxy[i]
                elif self.xyxy[2 + i] > background_size[i]:
                    dxy[i] = background_size[i] - self.xyxy[2 + i]
            self.xyxy = np.array(
                (
                    self.xyxy[0] + dxy[0],
                    self.xyxy[1] + dxy[1],
        ...
        if augment:
            ...
            for key, val in aug2prob_cp.items():
                if self.cls_name not in aug2unit[key]:
                    continue
                if p < val:
                    self.augment = key
                    break
        ...
        if (
            size / (h * w) < 0.3
            or self.xyxy[2] - self.xyxy[0] < 6
            or self.xyxy[3] - self.xyxy[1] < 6
        ):
    ...
    def draw(self, img: np.ndarray, inplace=True):
        ...
        if self.augment is not None:
            if self.augment == "trans":  # Transparency augmentation
                ...
                upimg = Image.fromarray(
                    np.concatenate([self.img, alpha[..., None]], -1)
                )
                upimg = np.array(
                    Image.alpha_composite(
                        Image.fromarray(subimg).convert("RGBA"), upimg
                    ).convert("RGB")
                )
                ...
                upimg = add_filter(
                    self.img, color, color2alpha[color], bright, replace=False
                )
    ...
    def show_box(self, img: Image, cls2color: dict | None = None):
        ...
        return plot_box_PIL(
            img, self.xyxy, text=self.get_name(), format="voc", box_color=color
        )


class Generator:
    towers_bc_pos: dict = (
        towers_bottom_center_grid_position  # with keys: ['king0', 'king1', 'queen0_0', 'queen0_1', 'queen1_0', 'queen1_1']
    )
    ...
    def __init__(
        ...
        map_update: dict | None = None,
        augment: bool = True,
        dynamic_unit: bool = True,
        avail_names: Sequence[str] = None,
        recognize_names: Sequence[str] = None,
        noise_unit_ratio: float = 0.0,
        dataset_root: str | Path | None = None,
        classes_yaml: str | Path | None = None,
        generation_filter: str = "bars",
    ):
        """
        Args:
          ...
          recognize_names: Specify the recognizable classes.
        ...
        self.path_manager = PathManager(dataset_root)
        self.path_segment = self.path_manager.path / "images/segment"
        if not self.path_segment.is_dir():
            raise FileNotFoundError(
                f"Segment dataset not found: {self.path_segment}. "
                "Pass dataset_root=... pointing to Clash-Royale-Detection-Dataset."
            )
        ...
        self.map_cfg.update(map_update or {"mode": "naive", "size": 5})
        ...
        self.recognize_names = recognize_names
        ...
        for p in sorted(self.path_segment.glob("*")):
            ...
            for p_img in p.glob("*.png"):
                assert (
                    p_img.name.split("_")[0] == p.name
                ), f"ERROR segment image name: {p_img}, make sure the prefix name of segment image is same as its parent directory name."
            for bel in range(2):
                p_name = f"{p.name}_{bel}"
                p_bel = str(p / (p_name + "*.png"))
                if len(glob.glob(p_bel)):
                    if (
                        avail_names is not None and p.name not in avail_names
                    ):  # noise unit
                        paths, u2i, i2u = (
                            self.noise_unit_paths,
                            self.noise_unit2idx,
                            self.idx2noise_unit,
                        )
                    else:
                        paths, u2i, i2u = (
                            self.moveable_unit_paths,
                            self.moveable_unit2idx,
                            self.idx2moveable_unit,
                        )
    ...
    def build_background(self):
        ...
        self.background = np.array(
            Image.open(
                ...
                / f"images/segment/backgrounds/background{background_index:02}.jpg"
            )
        )
        self.background_size = self.background.shape[:2][::-1]
        assert (
            self.background_size[0] == background_size[0]
            and self.background_size[1] == background_size[1]
        )
        ...
        if self.augment:
            ...
            if p < background_augment["prob"]:
                add_filter(
                    self.background, "red", alpha=100, xyxy=background_augment["xyxy"]
                )
                red_bound = Image.open(
                    str(self.path_segment / "backgrounds/red_bound.png")
                )
                self.background = np.array(
                    Image.alpha_composite(
                        Image.fromarray(self.background).convert("RGBA"), red_bound
                    ).convert("RGB")
                )

    @staticmethod
    def _max_intersect_ratio(
        xyxy: tuple | np.ndarray, box: List[np.ndarray] | np.ndarray
    ):
        ...
        inter_h = (
            np.minimum(max1[..., 0], max2[..., 0])
            - np.maximum(min1[..., 0], min2[..., 0])
        ).clip(0.0)
        inter_w = (
            np.minimum(max1[..., 1], max2[..., 1])
    ...
    def _intersect_ratio_with_mask(unit: Unit, mask: np.ndarray):
        ...
        if unit.mask.sum() == 0:
            return 0.0
        inter = mask[unit.xyxy[1] : unit.xyxy[3], unit.xyxy[0] : unit.xyxy[2]][
            unit.mask
        ].sum()
    ...
    def xyxy2cxcywh(self, xyxy, img_shape, relative=True):
        if xyxy.ndim == 1:
            xyxy = xyxy.reshape(1, 4)
        bx, by = np.round(xyxy[:, [0, 2]].sum(-1) / 2), np.round(
            xyxy[:, [1, 3]].sum(-1) / 2
        )
    ...
    def build(
        ...
        while True:
            ...
            for u in self.unit_list[::-1]:  # reverse order for NMS
                ...
                if u.cls_name in tower_unit_list:
                    if ratio > tower_intersect_ratio_thre:
                        keep_unit = False
                elif u.cls_name == "bar":
                    if ratio > bar_intersect_ratio_thre:
                        keep_unit = False
                elif ratio > self.intersect_ratio_thre:
                    keep_unit = False
                if not keep_unit:
        ...
        for u in unit_avail[::-1]:  # increase order for drawing
            ...
            if self.generation_filter == "bars" and u.cls_name in ["bar", "bar-level"] and u.states[0] == 0:
                # print(u.cls, u.xyxy, u.cls_name, u.states)
                write_this_box = False
                pass
            if self.generation_filter == "blue" and u.cls_name and u.states[0] == 1:
                # print(u.cls, u.xyxy, u.cls_name, u.states)
                write_this_box = False
                pass
        ...
        if show_box:
            ...
            for u in unit_avail:
                if u.cls_name in self.recognize_names:
        ...
        if generate_ann:
            if box_format != "cxcywh":
                raise ValueError("generate_ann=True requires box_format='cxcywh'")
            if not save_path:
                raise ValueError("generate_ann=True requires save_path")
            output_file = Path(save_path).with_suffix(".txt")
            write_yolo_annotations(box, output_file)
    ...
    def _build_unit_from_path(
        ...
        if max_width is not None:
            if isinstance(max_width, tuple):
                max_width = self._sample_range(*max_width)
        ...
        if "right" in xy_format:
            xy_bottom_center[0] -= img.shape[1] / 2 / cell_size[0]
        if (self.avail_names is not None) and (
            name.split("_")[0] not in self.avail_names
        ):
            drop = True
        unit = Unit(
            img=img,
            xy_bottom_center=xy_bottom_center,
            level=level,
    ...
    def add_tower(self, king=True, queen=True):
        if king:
            for i in range(2):  # is enermy?
                tower_paths = self.path_manager.search(
                    subset="images",
                    part="segment",
                    name="king-tower",
                    regex=f"king-tower_{i}",
                )
                ruin_paths = self.path_manager.search(
                    subset="images",
                    part="segment",
                    name="background-items",
                    regex=r"^king-tower-ruin_\d+.png",
                )
                paths = (
                    tower_paths
                ...
                paths = paths or tower_paths or ruin_paths
                if not paths:
                    continue
                path = self._sample_elem(paths)
                unit = self._build_unit_from_path(
                    path, self.towers_bc_pos[f"king{i}"], 1
                )
        ...
        if queen:
            for i in range(2):  # is enermy?
                for j in range(2):  # left or right
                    choices = []
                    for name, weight in tower_generation_ratio.items():
                        paths = self.path_manager.search(
                            subset="images",
                            part="segment",
                            name=name,
                            regex=f"{name}_{i}",
                    ...
                    ruin_weight = max(0.0, 1.0 - sum(tower_generation_ratio.values()))
                    if ruin_paths and ruin_weight:
                        choices.append((ruin_weight, ruin_paths))
                    if not choices:
                        continue
                    _, paths = random.choices(
                        choices,
                        weights=[weight for weight, _ in choices],
                        k=1,
                    )[0]
                    path = self._sample_elem(paths)
                    unit = self._build_unit_from_path(
                        path, self.towers_bc_pos[f"queen{i}_{j}"], 1
                    )
                    self._add_component(unit)
    ...
    def _update_map(
        ...
        assert mode in [
            "dynamic",
            "naive",
        ], f"map update mode must in ['dynamic', 'naive']"
        if not replace:
            a = a.copy()
        ...
        if mode == "dynamic":
            d = (
                np.stack(np.meshgrid(np.arange(size), np.arange(size)), -1) - size // 2
            )  # delta
            dxy = (
                (d + xy.reshape(1, 1, 2)).reshape(-1, 2).T
            )  # xy around indexs with sizexsize
            ...
            nonzero = (a[dx, dy] != 0) ^ (
                (dx == xy[0]) & (dy == xy[1])
            )  # get nonzeros mask in around and remove center index
    ...
    def _sample_from_map(
        self, level: int, noisy: bool = True, replace_map: bool = True
    ):
        """
        ...
        if not replace_map:
            map = map.copy()
        assert (
            isinstance(map, np.ndarray) and map.dtype == np.float32
        ), "The distribution map must be ndarray and float32"
        ...
        if noisy:
            ...
            xy = np.array(
                [np.clip(xy[0], 0, grid_size[0]), np.clip(xy[1], 0, grid_size[1])]
            )
        return xy
    ...
    def _add_component(self, unit: Unit):
        components = [
            key
            for key, (units, prob) in component2unit.items()
            if unit.cls_name in units
        ]
        if len(components) == 0:
            return
        ...
        for c in cs:
            if isinstance(c, tuple):
                ...
                # DEBUG:
                # c = 'bar'
            if c in component_cfg:
                cfg = component_cfg[c]
            else:
                cfg = component_cfg[c + str(unit.states[0])]
            center, dx_range, dy_range, max_width, xy_format = cfg
            if center == "bottom_center":
                center = unit.xy_cell
            elif center == "top_center":
                center = (
                    unit.xy_cell[0],
                    (unit.xyxy[1] - xyxy_grids[1]) / cell_size[1],
                )  # xyxy_grids[1] is the top of whold grid in image
            elif center == "center":
                center = (
                    unit.xy_cell[0],
                    ((unit.xyxy[1] + unit.xyxy[3]) / 2 - xyxy_grids[1]) / cell_size[1],
                )
            ...
            if len(paths) == 0:
                fallback_name = {
                    "tower-bar": "bar",
                    "king-tower-bar": "bar",
                    "king-tower-level": "bar-level",
                    "dagger-duchess-tower-bar": "bar",
                }.get(c)
                if fallback_name is not None:
                    paths = sorted(
                        (self.path_segment / fallback_name).glob(
                            f"{fallback_name}_{unit.states[0]}*"
                        )
                    )
            if len(paths) == 0:
                continue
            ...
            if c == "bar":  # update xy, xy is junction of bar-level and bar
                ...
                paths_bar_level = sorted(
                    self.path_manager.path.joinpath("images/segment/bar-level").glob(
                        f"bar-level_{unit.states[0]}*"
                    )
                )
                if paths_bar_level:
                    bar_level = self._build_unit_from_path(
                        self._sample_elem(paths_bar_level),
                        xy_,
                        level,
                        None,
                        "right_center",
            ...
            cu = self._build_unit_from_path(
                self._sample_elem(paths), xy, level, max_width, xy_format
            )
            ...
            if c == "dagger-duchess-tower-bar":
                ...
                icon_path = (
                    self.path_segment
                    / "background-items/dagger-duchess-tower-icon.png"
                )
                if icon_path.exists():
                    bar_icon = self._build_unit_from_path(
                        icon_path,
                        junction,
                        level,
                        None,
        ...
        for name, (prob, cfgs) in item_cfg.items():
            ...
            for cfg in cfgs:
                if pure_name not in background_item_list:
                    ...
                    paths = sorted(
                        (self.path_segment / "background-items").glob(file_name + "*")
                    )
                if not paths:
                    continue
                level = unit2level[
                    pure_name
                ]  # [0: background_items, 3: big-text, emote]
    ...
    def add_unit(self, n=1):
        ...
        def add_compoents(freq, drop, paths):
            ...
            idxs = self._sample_prob(freq, size=int(n * ratio), replace=True).reshape(
                -1
            )
            for i in idxs:
                p = paths[i]
                ...
                unit = self._build_unit_from_path(
                    self._sample_elem(sorted(glob.glob(p))), xy, level, drop=drop
                )
                self._add_component(unit)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate a synthetic YOLO dataset")
    parser.add_argument("--dataset-root", type=Path, required=True)
    parser.add_argument("--output", type=Path, default=Path("generation"))
    parser.add_argument("--count", type=int, default=100)
    parser.add_argument(
        "--units-min",
        type=int,
        default=10,
        help="Minimum number of random units per image (inclusive)",
    )
    parser.add_argument(
        "--units-max",
        type=int,
        default=15,
        help="Maximum number of random units per image (inclusive)",
    )
    ...
    if args.units_min < 0:
        parser.error("--units-min must be non-negative")
    if args.units_max < args.units_min:
        parser.error("--units-max must be greater than or equal to --units-min")
    ...
    generator = Generator(
        ...
        generation_filter=args.generation_filter,
    )
    image_names: list[str] = []
    for index in range(args.count):
        image_name = f"gen_{index + 1}.jpg"
        image_names.append(image_name)
        generator.add_tower()
        generator.add_unit(n=random.randint(args.units_min, args.units_max))
        generator.build(
            save_path=args.output / image_name,
            generate_ann=True,
        )
        generator.reset()

    (args.output / "yolo_annotations.txt").write_text(
