...
def init_device():
    ...
    sdk = subprocess.run(['adb', 'shell', 'getprop', 'ro.build.version.sdk'], 
                         capture_output=True, text=True).stdout.strip()
    
    # Push minicap binary and library
    subprocess.run(['adb', 'push', f'minicap/prebuilt/{abi}/bin/minicap', 
                    '/data/local/tmp/'], check=True)
    subprocess.run(['adb', 'push', f'minicap/prebuilt/{abi}/lib/android-{sdk}/minicap.so', 
                    '/data/local/tmp/'], check=True)
...
def start_minicap_process(port=1313, device_serial=None):
    ...
    subprocess.run(
        adb_prefix + ['forward', f'tcp:{port}', 'localabstract:minicap'],
        check=True
    )
...
class MinicapClient:
    ...
    def read_banner(self):
        ...
        print(f"Banner received - Version: {version}, Real size: {real_width}x{real_height}")
        return {
            'version': version, 'real_width': real_width, 'real_height': real_height,
...
def advanced_average_difference(video_path, buffer_size=5, threshold=30):
    ...
    while True:
        ...
        if len(frame_buffer) == buffer_size:
            ...
            cv2.putText(info_frame, f"Motion: {motion_percentage:.1f}%", (10, 30),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(info_frame, f"Diff mean: {diff_mean:.1f}", (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
class CardsCycle:
    ...
    def process_frame(self, img):
        ...
        if not self.in_battle:
            result_fps = 24
            img, nickname_matches = find_and_draw_pattern(img, self.nickname_im, threshold=0.8)
            img, specate_matches = find_and_draw_pattern(img, self.spectate_im, threshold=0.5)
            ...
            for nick in nickname_matches:
                for spectate in specate_matches:
                    if is_intersect(nick[0], nick[0] + nick[2], spectate[0], spectate[0] + spectate[2]):
                        x, y = (spectate[0] + spectate[2] // 2, spectate[1] + spectate[3] // 2)
...
if __name__ == '__main__':
    ...
    try:
        ...
        display_frames_process(port=1313, target_fps=24, callback=cards_cycle.process_frame)
